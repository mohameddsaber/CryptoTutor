...
public class DES {
    ...
    public String encrypt(String plainTextHex, String keyHex) {
        ...
        String permuted = applyPermutation(plainBin, IP);
        ...
        String combined = R + L;
        String cipherBin = applyPermutation(combined, FP);
    ...
    public String decrypt(String cipherTextHex, String keyHex) {
        ...
        String keyBin = hexToBinary(keyHex);
        String permuted = applyPermutation(cipherBin, IP);
        String L = permuted.substring(0, 32);
        String R = permuted.substring(32);
        String[] subkeys = generateSubKeys(keyBin);
        for (int i = 0; i < 16; i++) {
            String temp = R;
            R = xor(L, feistel(R, subkeys[15 - i]));
            L = temp;
        }
        String combined = R + L;
        String plainBin = applyPermutation(combined, FP);
