package Security;

/**
 * AES-128 Implementation
 * Supports Hexadecimal strings starting with "0x"
 */
public class AES {

    private static final int NB = 4; // Block size in words
    private static final int NK = 4; // Key length in words
    private static final int NR = 10; // Number of rounds

    private static final int[] SBOX = {
            
    };

    private static final int[] INV_SBOX = new int[256];
    static {
        for (int i = 0; i < 256; i++) INV_SBOX[SBOX[i]] = i;
    }

    private static final int[] RCON = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

    // --- Core API ---

public static String encrypt(String hexText, String hexKey) {

    int[][] state = createStateMatrix(hexText);

    int[] keyWords = generateKeySchedule(hexKey);

    addRoundKey(state, getRoundKeyMatrix(keyWords, 0));

    for (int round = 1; round <= 9; round++) {

        subBytes(state);

        shiftRows(state);

        mixColumns(state);

        addRoundKey(state, getRoundKeyMatrix(keyWords, round));
    }

    // Final round
    subBytes(state);

    shiftRows(state);

    addRoundKey(state,
            getRoundKeyMatrix(keyWords, 10));

    return stateToHex(state);
}

public static String decrypt(String hexText, String hexKey) {

    int[][] state = createStateMatrix(hexText);

    int[] keyWords = generateKeySchedule(hexKey);

    // Initial round with last key
    addRoundKey(state, getRoundKeyMatrix(keyWords, 10));

    for (int round = 9; round >= 1; round--) {

        invShiftRows(state);

        invSubBytes(state);

        addRoundKey(state, getRoundKeyMatrix(keyWords, round));

        invMixColumns(state);
    }

    // Final round
    invShiftRows(state);

    invSubBytes(state);

    addRoundKey(state, getRoundKeyMatrix(keyWords, 0));

    return stateToHex(state);
}

    // --- AES Transformations ---

    // --- 1. SUB BYTES ---
private static void subBytes(int[][] state) {

    for (int r = 0; r < 4; r++) {

        for (int c = 0; c < 4; c++) {

            state[r][c] = SBOX[state[r][c]];
        }
    }
}
private static void invSubBytes(int[][] state) {

    for (int r = 0; r < 4; r++) {

        for (int c = 0; c < 4; c++) {

            state[r][c] = INV_SBOX[state[r][c]];
        }
    }
}

// --- 2. SHIFT ROWS ---
private static void shiftRows(int[][] state) {

    // Row 1 -> shift left by 1
    int temp = state[1][0];

    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = temp;

    // Row 2 -> shift left by 2
    temp = state[2][0];
    int temp2 = state[2][1];

    state[2][0] = state[2][2];
    state[2][1] = state[2][3];
    state[2][2] = temp;
    state[2][3] = temp2;

    // Row 3 -> shift left by 3
    temp = state[3][3];

    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
    state[3][0] = temp;
}
private static void invShiftRows(int[][] state) {

    // Row 1 -> shift right by 1
    int temp = state[1][3];

    state[1][3] = state[1][2];
    state[1][2] = state[1][1];
    state[1][1] = state[1][0];
    state[1][0] = temp;

    // Row 2 -> shift right by 2
    temp = state[2][0];
    int temp2 = state[2][1];

    state[2][0] = state[2][2];
    state[2][1] = state[2][3];
    state[2][2] = temp;
    state[2][3] = temp2;

    // Row 3 -> shift right by 3
    temp = state[3][0];

    state[3][0] = state[3][1];
    state[3][1] = state[3][2];
    state[3][2] = state[3][3];
    state[3][3] = temp;
}
// --- 3. MIX COLUMNS ---
private static void mixColumns(int[][] state) {

    for (int c = 0; c < 4; c++) {

        int s0 = state[0][c];
        int s1 = state[1][c];
        int s2 = state[2][c];
        int s3 = state[3][c];

        state[0][c] = mul2(s0) ^ mul3(s1) ^ s2 ^ s3;

        state[1][c] = s0 ^ mul2(s1) ^ mul3(s2) ^ s3;

        state[2][c] = s0 ^ s1 ^ mul2(s2) ^ mul3(s3);

        state[3][c] = mul3(s0) ^ s1 ^ s2 ^ mul2(s3);
    }
}
private static void invMixColumns(int[][] state) {

    for (int c = 0; c < 4; c++) {

        int s0 = state[0][c];
        int s1 = state[1][c];
        int s2 = state[2][c];
        int s3 = state[3][c];

        // 9*x  = 2(2(2x)) ^ x
        int mul9s0  = mul2(mul2(mul2(s0))) ^ s0;
        int mul9s1  = mul2(mul2(mul2(s1))) ^ s1;
        int mul9s2  = mul2(mul2(mul2(s2))) ^ s2;
        int mul9s3  = mul2(mul2(mul2(s3))) ^ s3;

        // 11*x = 2(2(2x)) ^ 2x ^ x
        int mul11s0 = mul2(mul2(mul2(s0))) ^ mul2(s0) ^ s0;
        int mul11s1 = mul2(mul2(mul2(s1))) ^ mul2(s1) ^ s1;
        int mul11s2 = mul2(mul2(mul2(s2))) ^ mul2(s2) ^ s2;
        int mul11s3 = mul2(mul2(mul2(s3))) ^ mul2(s3) ^ s3;

        // 13*x = 2(2(2x)) ^ 2(2x) ^ x
        int mul13s0 = mul2(mul2(mul2(s0))) ^ mul2(mul2(s0)) ^ s0;
        int mul13s1 = mul2(mul2(mul2(s1))) ^ mul2(mul2(s1)) ^ s1;
        int mul13s2 = mul2(mul2(mul2(s2))) ^ mul2(mul2(s2)) ^ s2;
        int mul13s3 = mul2(mul2(mul2(s3))) ^ mul2(mul2(s3)) ^ s3;

        // 14*x = 2(2(2x)) ^ 2(2x) ^ 2x
        int mul14s0 = mul2(mul2(mul2(s0))) ^ mul2(mul2(s0)) ^ mul2(s0);
        int mul14s1 = mul2(mul2(mul2(s1))) ^ mul2(mul2(s1)) ^ mul2(s1);
        int mul14s2 = mul2(mul2(mul2(s2))) ^ mul2(mul2(s2)) ^ mul2(s2);
        int mul14s3 = mul2(mul2(mul2(s3))) ^ mul2(mul2(s3)) ^ mul2(s3);

        state[0][c] = mul14s0 ^ mul11s1 ^ mul13s2 ^ mul9s3;

        state[1][c] = mul9s0 ^ mul14s1 ^ mul11s2 ^ mul13s3;

        state[2][c] = mul13s0 ^ mul9s1 ^ mul14s2 ^ mul11s3;

        state[3][c] = mul11s0 ^ mul13s1 ^ mul9s2 ^ mul14s3;
    }
}

// --- 4. ADD ROUND KEY ---
private static void addRoundKey(int[][] state, int[][] roundKey) {

    for (int r = 0; r < 4; r++) {

        for (int c = 0; c < 4; c++) {

            state[r][c] ^= roundKey[r][c];
        }
    }
}

    // --- Math & Key Expansion ---
private static int[] generateKeySchedule(String hexKey) {

    int[] w = new int[44];

    for (int i = 0; i < 4; i++) {
        w[i] = Integer.parseUnsignedInt(
                hexKey.substring(i * 8, i * 8 + 8), 16);
    }

    for (int i = 4; i < 44; i++) {

        int temp = w[i - 1];

        if (i % 4 == 0) {
            temp = subWord(rotWord(temp)) ^ (RCON[i / 4] << 24);
        }

        w[i] = w[i - 4] ^ temp;
    }

    return w;
}
private static int mul2(int b) {
    int shifted = (b << 1) & 0xFF;
    return (b & 0x80) != 0 ? (shifted ^ 0x1B) : shifted;
}

private static int mul3(int b) {
    return mul2(b) ^ b;
}

private static int rotWord(int word) {
    return (word << 8) | (word >>> 24);
}

private static int subWord(int word) {

    int b0 = SBOX[(word >>> 24) & 0xFF];
    int b1 = SBOX[(word >>> 16) & 0xFF];
    int b2 = SBOX[(word >>> 8) & 0xFF];
    int b3 = SBOX[word & 0xFF];

    return (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;
}
private static int[][] getRoundKeyMatrix(int[] keyWords, int round) {
    int[][] roundKey = new int[4][4];
    for (int c = 0; c < 4; c++) {
        int word = keyWords[round * 4 + c];
        roundKey[0][c] = (word >>> 24) & 0xFF;
        roundKey[1][c] = (word >>> 16) & 0xFF;
        roundKey[2][c] = (word >>> 8) & 0xFF;
        roundKey[3][c] = word & 0xFF;
    }
    return roundKey;

}
private static int[][] createStateMatrix(String hexText) {
    int[][] state = new int[4][4];
    for (int i = 0; i < 16; i++) {
        state[i % 4][i / 4] = Integer.parseUnsignedInt(
                hexText.substring(i * 2, i * 2 + 2), 16);
    }
    return state;
}
private static String stateToHex(int[][] state) {
    StringBuilder sb = new StringBuilder();

    for (int c = 0; c < 4; c++) {
        for (int r = 0; r < 4; r++) {
            sb.append(String.format("%02X", state[r][c]));
        }
    }
    return sb.toString();
}    
}